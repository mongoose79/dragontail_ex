...
func TestEditRestaurant(t *testing.T) {
	...
	request, err := http.NewRequest("PUT", url, strings.NewReader(`{"name":"Giraffe1",
		"type":"Asian1","phone":"123","location":"32.109805/34.840232"}`))
	assert.NoError(t, err)

	response := utils.InvokeRequest(request, EditRestaurant, "/restaurant/{name}")
	assert.Equal(t, http.StatusOK, response.Code)
	...
	assert.Equal(t, "Giraffe1", products[1].Name)
	assert.Equal(t, "Asian1", products[1].Type)
	assert.Equal(t, "123", products[1].Phone)
